...
1. **Model-rated-noise shows at the bottom of To-dos (RESOLVED, Vicky's call 2026-07-27).** A `gmail_message`/`gmail_transactional` the model rates noise is kept in the feed and returned by `list_feed`; To-dos renders it below `can_wait`, so the Later material the refresh already identified is visible in the feed. It is NOT re-shown in the Later tab: the Day ring ignores noise client-side (`YourDayScreen` filters `tier !== 'noise'`), and `_on_home_refs` excludes every stored ref, so a stored noise item is on Home and therefore out of Later's residual. Deterministic noise (newsletters, `ci_ok`) is still dropped at ingest and lives only in Later's live view. (An earlier draft of this doc said noise should route *out* of Home to Later; that is reversed here.)
